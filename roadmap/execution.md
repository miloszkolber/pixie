...
Read [README.md](README.md), [contracts.md](contracts.md), [feature-coverage.md](feature-coverage.md), [acceptance.md](acceptance.md), relevant reviews and the assigned feature plan. Implement the tasks rather than writing another replacement plan. Recheck the actual checkout; the third review starts from the second-pass roadmap branch, not the older alternative roadmap.

## Working rules

Use the specified defaults and one owner per state/delivery/runtime concern. Preserve user changes, unknown native records, durable authority and retained behavior. Test with disposable state and owned/licensed fixtures, not live credentials, documents or repositories. Read reviews as evidence; current requirements live in the contracts/plans and only this ledger records progress.
...
Record revision and concurrent changes; recheck F01–F38 against source. Capture current browser/host/native behavior separately and preserve correct tests. The third pass's copied-unit/command probes confirm existing behavior, not fixed production behavior. Promote them into real endpoint/subprocess regressions and invert unsafe expectations when implementing fixes.

The roadmap changes no runtime, dependencies or workflows. Current operating docs remain true until corresponding behavior ships. Native bridge, services, workers, complete application and release artifacts still need their own live evidence.

## Task ledger

Every task below remains open until implementation evidence is recorded. FC01–FC33 and X01–X14 are required traceability, not optional tests. Discovering another retained method extends its appropriate row rather than deleting the method as cleanup.

### Regressions and contracts

- [ ] FIX-01 — F/B: real production restart exits through bounded teardown and obtains a new process/boot identity (F02).
...
- [ ] FIX-07 — B/F: agent content revisions, exclusive create, rooted identity and stale/external-edit handling with explicit writer assumptions (F23).
- [ ] FIX-08 — B/F: literal-loopback assistant bind and validated startup inputs (F26).
...
- [ ] FIX-12 — F/G: controller child-group termination and finite pipe draining, including Git and final container reaping; X03 (F31).
- [ ] FIX-13 — F/A/E: typed publication/durability outcomes, reconciliation and post-rename fault injection; X04 (F32).
- [ ] API-01 — A: exhaustive browser/controller/host/native catalog and FC mapping, including schedule methods and removal of broad Administration/provider assumptions (F20).
...
- [ ] MIG-01 — A/B/F: full metadata inventory/conversion/rollback, ungrouped queues/deletions, old archive metadata and no ledger rewind (F25/F32).
...
- [ ] BUILD-05 — G/F: whole-composition drain/restart, both unit choices, effective container descendant handling and mode switch.
...
- [ ] EXT-02 — E: generalized registry, complete-map persistence outcomes, independent desired/readiness states and no lock-held startup; invalid restrictive config fails locally.
...
- [ ] PKG-03 — G/F: real matching Docker+assistant and full-host artifacts, both architectures, upgrade/rollback/mode-switch and old-browser recovery.
- [ ] DOC-01 — G/A: root guidance/commands/contracts track implementation and canonical owners.
- [ ] DOC-02 — G: brief current-state docs, no superseded plans, checked links/anchors/examples and supported installation profiles.
...
**Gate 1 — Contracts/regressions.** Existing and third-pass fixtures, exhaustive FC catalog, separated request domains, v2 capability/authority, route ownership, selection and shared-build seam are agreed. BRIDGE feasibility begins here.
