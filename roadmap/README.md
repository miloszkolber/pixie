...
- `check-package-artifacts` consumes the staged bundle and verifies the four commit-named archives, their unit/config contents, the static command sources and the executed `--version`/`doctor` probes for both architectures. The `packageArtifacts` reduction group exempts only readiness, lifecycle, uninstall, host embedded-UI and the full-host native-engine facade; a reduced row is reported as reduced, never as passing, and every structural row still fails closed.
...
9. **CI evidence pipeline:** the artifact stage is done — the archive/OCI collectors, the coverage/performance producer, the native arm64 performance leg and the per-architecture probe merge are wired, and the four arm64 `--version`/`doctor` rows are un-reduced. Remaining: an authorized publication rehearsal.
10. **Post-publish verification:** `verify-publication` runs after `publish` and checks the Git tag, the published release assets and checksums, and the published registry index/platform digests and SBOM/provenance attestations. Decision recorded: the pre-publish `releaseGate` registry rows stay reduced because registry state cannot exist before publication, so the release is verified in a second phase rather than by un-reducing the first. `latest` promotion remains out of scope because the workflow never promotes it; adopting it needs verified source ancestry.
