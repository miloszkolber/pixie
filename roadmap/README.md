...
- `release-gate` lacks release identity, archives, binaries, OCI platform digests, checksums, SBOM/provenance, source reachability, and publication authorization. CI produces archives and the OCI tar first and the evidence collector ingests them; the remaining blockers are non-local Git tag/GitHub Release/registry provenance/latest-promotion inputs and complete platform digests.
...
- **D3 Browser: gate done.** The module is disabled by default and `SetEnabled(browser, true)` fails closed until `SetWorkerBoundaryVerified(true)` is called by a real boundary implementation. That external worker boundary does not exist yet, so untrusted Browser stays unavailable.
...
- **Machine-readable reductions:** `package/contracts/reductions.json` records every reduced FC/X/gate with its reason and the operator approval, and the performance legacy-field/arm64-target reductions. `package/scripts/produce-evidence-inputs.ts` merges them into the emitted coverage/performance inputs. A reduction removes the legacy live-matrix requirement, not the retained feature, and it never records a reduced row as passing.
