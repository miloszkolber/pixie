...
These are target requirements, not shipped functionality. The latest review baseline is `eacfac43e81e04e798f4e8fd43af7d77315ad6ff` on the roadmap branch; recheck the checkout and concurrent changes. The [first](repository-review.md), [second](second-pass-review.md) and [third](third-pass-review.md) reviews record evidence. Only execution.md records implementation status; contracts and feature plans contain the current requirements.
...
[Builds and releases](builds-and-releases.md) owns exact names, continuous-release approval, candidate tests, immutable retries, provenance, services and mode switching. This review does not alter or dispatch publication workflows. Publication approval does not imply live deployment approval.
...
| [acceptance.md](acceptance.md) | Cross-boundary tests mapped to existing feature rows and tasks |
...
Use [sources.md](sources.md) and [draft-review.md](draft-review.md) for pinned evidence and attributed experiments. Do not recreate removed planning files in docs/ or add competing copies. Current operating documentation remains under docs/ and changes when behavior ships.
...
> Implement roadmap/README.md using execution.md, contracts.md, feature-coverage.md and acceptance.md. Work through dependencies and record actual verification. Preserve installed Pi, native state and retained features. Publish both host variants and the Docker image under the same commit release ID only under the authorized release policy. Canvas and Openfig are the last feature stages.
