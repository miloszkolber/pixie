...
- `check-performance` measures 2 of 4 required process targets (the assistant and full-host binaries on amd64 with fresh-process startup p50/p95 and peak RSS) and records the legacy worker/decoded-buffer fields and the two arm64 targets as explicit approved reductions. arm64 performance remains unproven.
...
- **D3 Browser: worker boundary implemented at source.** The module is disabled by default and `SetEnabled(browser, true)` fails closed until a verified external worker boundary exists. `package/cmd/browser-worker/main.go` serves `GET /isolation` (fact-only report) and `POST /run` (bounded operations) under `package/systemd/pixie-browser-worker.service`; on startup the controller probes `PIXIE_BROWSER_WORKER_URL` and calls `SetWorkerBoundaryVerified(true)` only when every required containment fact is proven (`package/internal/mcpserver/worker_boundary.go`, `package/internal/browser/isolation.go`). Without a URL, a non-conforming report or a probe failure, Browser stays unavailable. Live systemd enforcement and routing every module operation through the worker are not yet exercised.
...
8. **Browser containment boundary — SEC-02/EXT-03, FC31, X12/X13. Implemented at source; live host unproven.** Chromium still uses `--no-sandbox` and the in-process path still shares the controller UID, writable state and host network, so that path stays gated. A separate worker (`package/cmd/browser-worker/main.go`) serves `GET /isolation` and `POST /run` and is deployed under `package/systemd/pixie-browser-worker.service` with a dedicated user, `NoNewPrivileges`, `PrivateTmp`, `ProtectSystem=strict`, `ProtectHome`, `MemoryMax`, `CPUQuota`, `TasksMax`, `IPAddressDeny=any` plus targeted `IPAddressAllow`, and a bounded stop. The controller probes `PIXIE_BROWSER_WORKER_URL` on startup and unlocks the module only when the report proves version, a distinct non-root uid/gid, filesystem mediation, restricted egress, pre-exec limits and bounded cleanup; otherwise it fails closed (`package/internal/mcpserver/worker_boundary.go`). Remaining: exercise the systemd profile and its kernel enforcement on a real host, and route every module operation (not only panels and artifacts) through the worker.

9. **CI evidence order — REL-01–REL-04/COVERAGE-01/PKG-01–PKG-03/PERF-01. Sequenced; producer wired.** Static validation no longer runs the live gates. `stage` and `image` produce exact-commit archives and the OCI tar first; the `evidence` job downloads both, runs `produce-evidence-inputs.ts`, collects the versioned bundle (D7), and runs coverage/package/performance/release gates; `publish-image` and `publish` depend on that evidence job. Artifact names use the identity job's source commit instead of `github.sha`. The producer emits FC01 live evidence from executed staged-binary probes, fresh-process amd64 startup measurements, and the committed operator-approved reductions; `collect-evidence.ts` inspects the four archives, the staged local `release-manifest.json` and the docker-save/OCI image tar, hashes the contained binaries, and feeds `check-package-artifacts --evidence` and `release-gate --evidence`. Both gates load the committed `packageArtifacts`/`releaseGate` reductions, report reduced rows as reduced instead of passing, and keep structural archive/systemd/config checks, static workflow policy, source reachability, archive/binary identity and checksum consistency mandatory. Still missing and explicitly reduced or fail-closed: full per-row live coverage, non-native arm64 execution, packaged-binary lifecycle/uninstall/embedded-UI and full-host facade probes, Git tag/GitHub Release/registry provenance/SBOM/latest evidence, arm64 performance and arm64 platform digests.
...
| SEC-02/PERF-01 | Partial | The verified external worker boundary exists at source with fail-closed controller verification; live systemd enforcement and four live performance profiles are still absent. |
...
7. **Enforce Browser separation:** the separate worker UID/filesystem/network design and systemd deployment profile now exist (`package/cmd/browser-worker`, `package/systemd/pixie-browser-worker.service`) and the controller defaults Browser unavailable until the worker report verifies. Remaining: prove the systemd kernel enforcement live and route every module operation through the worker.
