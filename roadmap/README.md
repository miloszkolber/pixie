...
| SEC-02/PERF-01 | Partial | Browser MCP registration and probe are implemented at source with fail-closed controller behavior; live host verification of registration, probe and the optional Compose service is absent, as are four live performance profiles. |
...
7. **Verify the browser-MCP pointer:** the controller registration/probe and the Settings → Browser surface are implemented, and Compose ships an optional isolated `pixie-browser` service. Remaining: verify registration, probe and fail-closed behavior against a real assistant and prove the Compose service's isolation live; the endpoint's hardening and egress remain the deployment's responsibility.
