...
6. **Port retained native integrations:** implement the public bridge and each FC13–FC28 operation through Go against independent npm and standalone Pi distributions. Prioritize the operations the verified 0.85.1 RPC surface already exposes: `fork`, `clone`, `set_model`, `get_available_models`, `set_thinking_level`, `get_available_thinking_levels`, `get_messages`, `get_session_stats`, `get_entries`, `export_html`, `switch_session`, `compact`, `steer`, `follow_up` and `clear_queue`. Keep the Bun oracle until exact parity or explicit named reductions are approved.
