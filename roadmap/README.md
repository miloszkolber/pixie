...
- `check-coverage` has no mapped live evidence for the 28 mandatory FC rows, 14 X rows, or Gates 1–5. The CI evidence job now stages the exact-commit archives and OCI tar, executes the amd64 binaries, and fails closed on those missing rows rather than fabricating them; the repaired behaviors have focused Go regression tests, not per-row live coverage.
...
6. **Port retained native integrations:** the vanilla RPC slice landed (session.configure, fork/clone, getMessages, stats, compact, rename, commands, steer, followUp, clearQueue, switch). Remaining: (a) raw `extension_ui_request`/`extension_ui_response` frames and their controller projection for FC13/FC14; (b) filesystem `pi.sources.*`/`pi.agent-mentions.list` for FC23; (c) an explicitly opt-in bridge sidecar using the selected installation's public SDK (`ModelRuntime`, `SettingsManager`, `DefaultPackageManager`, `AgentSession`/`ExtensionAPI`, `pi-mcp-adapter` events) for FC17–FC21 and FC26; (d) named reductions for FC15, the FC16 read path, `pi.tools.call` and `session.prompt.resource`. Keep the Bun oracle until exact parity or explicit named reductions are approved, then add a non-test binding that fails when a controller-sent method is absent or false in `nativeOperationSet`.
