...
- **Race suite status:** the full `-race` suite is environmentally blocked — `CGO_ENABLED=0` rejects `-race` unconditionally and `CGO_ENABLED=1` needs `gcc`, which is absent and uninstallable in this container. The non-race functional sweep passes (`CGO_ENABLED=0 go test -count=1` across controller, persist, workspace, mcpserver, piprotocol, security, diagnostics, and browser trees, all ok), with `go vet` and `gofmt` clean and a static concurrency review finding no unsynchronized shared state. Full race evidence remains a later-stage live-environment item.
