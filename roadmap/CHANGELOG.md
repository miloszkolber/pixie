...
- **Suite state after repair:** `bun test tests/pixie-assistant` 204 pass / 0 fail; `bun test tests/contracts` 12 pass; `bun test tests/webui` 367 pass / 23 fail (all 23 are the pre-existing `NameTooLong` data-URL harness failures in renderer tests, unrelated to roadmap changes); `bun test tests/pi-native-parity` 116 pass / 2 fail (both require live native-child/signet services, matching the documented pre-existing child-runtime boundary).
