...
### Core finishing with Canvas/Design parked

- **WebUI renderer harness fix (`9201963`):** the SSR test helper loads bundled output from a disposable temp file URL instead of a base64 data-URL specifier, fixing 23 `NameTooLong` resolver failures in chat and tool renderer suites. Verified by `bun test tests/webui` (421 pass, 0 fail, 3405 expectations, Bun 1.3.14), `bun run typecheck` (contracts, web `svelte-check` 0 errors and 0 warnings, tests, assistant), `bun run check:filenames` (OK), and `git diff --check` (clean). No product, contract, Go, Canvas, or Design code changes; no tests deleted. Browser QA and production screenshots remain open.
- **Race suite status:** the full `-race` suite is environmentally blocked — `CGO_ENABLED=0` rejects `-race` unconditionally and `CGO_ENABLED=1` needs `gcc`, which is absent and uninstallable in this container. The host has GCC 15.2.0 but no `go` executable, so it cannot supply the missing race runtime either. The non-race functional sweep passes (`CGO_ENABLED=0 go test -count=1` across controller, persist, workspace, mcpserver, piprotocol, security, diagnostics, and browser trees, all ok), with `go vet` and `gofmt` clean and a static concurrency review finding no unsynchronized shared state. Full race evidence remains a later-stage live-environment item.
- **Remaining live blockers:** Gate 5 needs actual both-architecture final binary runs, both-deployment systemd and Docker behavior, runnable per-platform image runs with published index and platform digests, real `sha-<12>` tag and Release publication under explicit `PIXIE_RELEASE_ENABLED` authorization, live npm versus standalone Pi profiles, live Chromium browser QA, repeated full-process performance measurements, and complete FC and X live rows — checkers (`check-coverage`, `check-performance`, `check-package-artifacts`, `release-gate`) correctly fail closed until that evidence exists. Canvas and Design stay parked with no paths touched by this batch.

### Local release-candidate and deployment evidence

- **Portable deterministic archive staging (`31aef5b`):** `build-release.ts` no longer depends on GNU tar options rejected by BusyBox. The in-repository regular-file-only USTAR writer orders byte names, fixes modes, UID/GID, and commit mtime, rejects symlinks, and writes a fixed gzip header. `bun test tests/build` passes (51 pass, 318 expectations); two clean local stages of `sha-31aef5b3bebc` produced byte-identical hashes for all four amd64/arm64 assistant and full-host archives. This is local staging only: manifests remain `completeSet:false` and no artifacts were published.
- **Native amd64 candidate execution (`31aef5b`):** extracted assistant and full-host archives report `sha-31aef5b3bebc` and the full 40-character revision. An isolated loopback assistant starts with an explicit test secret and no selected Pi; `/livez` and authorized `/readyz` return 200. An isolated full-host candidate serves `/livez` and its embedded UI; `/readyz` correctly reports 503 because no Pi is selected, so this does not claim native capability parity.
- **Transient systemd and OCI candidates (`31aef5b`):** a temporary `Type=exec`, `User=core`, `KillMode=mixed`, 30-second-stop full-host unit starts from the staged binary with isolated data, serves `/livez` and embedded UI, and stops without changing the active source-based `pixie-assistant.service`. A local non-published amd64 controller-only image builds from the final Docker target, reports the expected `tini` entrypoint and OCI version/revision labels, and runs healthy under a read-only rootfs, tmpfs, isolated data mount, explicit loopback public-origin test policy, and no local Pi settings; `/livez` and embedded UI return 200. The test container, local image, staged shared artifacts, and isolated data were removed after verification.
- **Remaining boundary:** these are native-amd64, local, no-Pi candidate checks. They do not supply arm64 execution, multi-platform OCI index or platform digests, an installed packaged user unit, upgrade/rollback/uninstall, a selected independent npm/standalone Pi profile, Browser live QA, full race support, performance records, GitHub Release/SBOM/provenance, or authorized publication/deployment. The host’s active `pixie-assistant.service` remains the pre-existing source-based Bun unit and was observed healthy but intentionally not replaced.

## 2026-09-12

### QA repair — portable diagnostics and packaged workspace acceptance

- **Implementation:** `94a033b` makes the TERM-ignoring diagnostics fixture survive shell `exec` differences, waits for the prompt worker’s retained reference before asserting the stable uncertain-idle-release refusal, adds vertical rail ArrowUp/ArrowDown/Home/End focus behavior, and makes Settings retain a valid local tab while falling back to resolved System state when profile capabilities disappear. Packaged acceptance now follows the current rail, mobile pane, Settings and close-chat contracts; it covers keyboard tab selection, 320×520 Settings geometry, responsive Pi sections and draft/recovery flows. Fixture shutdown is bounded after graceful TERM.
- **Behavior evidence:** the packaged Chromium fixture completed the workspace, reconnect, narrow-layout, Settings/System, five-width light/dark Pi settings, keyboard, and rejected prompt/credential recovery flows. A focused settings regression proves a vanished Pi selection resolves to System rather than the first pending tab. The previous immediate idle-release assertion now tolerates only the brief safe `busy` state and requires the final `uncertain` refusal.
- **Verification:** host Go 1.27.1 `CGO_ENABLED=1 go test -race -count=1 ./...` passed; `TestStopQuiescenceUncertainOnTimeout` passed 100 race iterations. A clean Docker `web-build` reported TypeScript/Svelte checks with 0 errors and 0 warnings, 181 artifacts, and a 226196/500000-byte initial bundle. `ui-acceptance` image candidate v32 exited 0 with both `Pi UI acceptance passed` and `UI acceptance passed`. Isolated Bun verification passed `bun run lint` and `pi-tools-settings.test.ts` (8 pass).
- **Remaining boundary:** this is local amd64 fixture coverage, not independent selected npm/standalone Pi, arm64, live service lifecycle, publication, release artifacts, or performance/coverage gate evidence. No active service, deployment, or publication changed.
