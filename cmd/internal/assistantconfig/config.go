// Package assistantconfig reads the private v2 service configuration for the
// pixie host launcher (`pixie serve`). It intentionally contains no Pi
// package selector: the archive launcher provides that through a private env var.
...
func Load(path string) (Config, error) {
	...
	if _, selected := object["piPackage"]; selected {
		return Config{}, errors.New("assistant config must not select piPackage; pixie supplies the bundled Pi archive")
	}
