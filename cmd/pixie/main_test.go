...
func TestServeArgumentsRequireAbsoluteConfig(t *testing.T) {
	parsed, err := parseServeArguments([]string{"--config", "/data/.db/pixie/assistant.json"})
	if err != nil || parsed.configPath != "/data/.db/pixie/assistant.json" {
		t.Fatalf("serve args = %#v, %v", parsed, err)
	}
	for _, values := range [][]string{
		...
		if _, err := parseServeArguments(values); err == nil {
			t.Fatalf("serve args %q were accepted", values)
		}
	}
}

func TestVersionLinesReportPixieReleaseAndPiVersion(t *testing.T) {
	version, revision = "sha-abc123", "deadbeef"
	defer func() { version, revision = "0.0.0-dev", "unknown" }()
