...
/** The two public archive products from build-release.ts. */
...
const ARCHIVE_PATTERN = /^(pixie_web|pixie)-sha-[0-9a-f]{12}-linux-(amd64|arm64)\.tar\.gz$/;
...
function checkUnit(
	...
	if (name === "pixie.service") {
		if (!/^\s*ExecStart=.*\bpixie\b.*\bserve\b.*--config\s+\S+/m.test(source)) {
			violations.push(`${name}: ExecStart must run pixie serve with an explicit assistant config`);
		}
	}
...
function checkArchives(
	...
	if (expected.length === 0) {
		liveRows.push({
			...
			message: "four commit-named public product archives (two products, linux amd64 and arm64)",
		});
	} else {
...
export async function collectPackageArtifactInput(
	...
	const commandSources = {
		...(await collectTextTree(repositoryRoot, "cmd", new Set([".go"]))),
		...(await collectTextTree(repositoryRoot, "internal", new Set([".go"]))),
	};
	const webuiSources = await collectTextTree(repositoryRoot, "webui", new Set([".go"]));
	...
	const facadeSources = await collectTextTree(repositoryRoot, "src/assistant", new Set([".ts"]));
