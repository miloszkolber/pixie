 ...
 *
 * Enumerates the allowed edges between the three logical source roots in the
 * single source tree and fails when a checked file crosses a forbidden edge:
 *
 *   assistant -> assistant, shared   (never web)
 *   shared    -> shared               (never web or assistant)
 *   web       -> web, shared          (never assistant)
 *
 * Logical roots map to directories: assistant owns `src/assistant` (and its
 * tests), shared owns `src/shared`, `schema` and `piprotocol` (and its
 * tests), and web owns everything else checked (`cmd`, `internal`,
 * `scripts`, `webui`, `systemd`, remaining tests).
 *
...
/** Go package paths map to their owning source root. The piprotocol package is
 * a subpath of the module path, so it must be tested first. */
const goModulePaths: readonly { readonly base: string; readonly root: SourceRoot }[] = [
	{ base: "github.com/miloszkolber/pixie/piprotocol", root: "shared" },
...
/** Returns the logical source root that owns an absolute path, if any. */
function rootOf(root: string, path: string): SourceRoot | null {
	const rel = relative(root, path).split(sep).join("/");
	for (const sourceRoot of sourceRootNames) {
		for (const prefix of rootPrefixes[sourceRoot]) {
			if (rel === prefix.slice(0, -1) || rel.startsWith(prefix)) return sourceRoot;
		}
	}
	return null;
}
...
async function checkManifests(root: string, violations: Violation[]): Promise<number> {
	let checked = 0;
	const candidates = [resolve(root, "package.json"), resolve(root, "webui", "package.json")];
	...
	for (const manifestPath of candidates) {
		try {
			...
			// A missing manifest is reported by other gates.
		}
	}
