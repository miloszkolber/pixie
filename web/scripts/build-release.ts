...
function productUsesRuntime(product: ReleaseProduct): boolean {
	return product === "pixie_cli" || product === "pixie";
}
...
export function installInstructions(product: ReleaseProduct): string {
	switch (product) {
		...
		case "pixie_cli":
			return `# pixie_cli\n\nKeep pixie, pixie_cli, libexec/pixie_assistant.js, and runtime/ together in one extracted directory. Run the native Pi TUI with ./pixie and the bundled host with:\n\n    ./pixie_cli serve --config /absolute/path/to/assistant.json\n\nDo not co-install this product with the full pixie archive: both provide the global pixie command and use the same Pi agent directory. Replace one installation only after stopping its service and TUI. The archive supplies its own Bun and Pi runtime; external Pi selection is rejected.\n`;
		case "pixie":
			return `# pixie\n\nKeep pixie, libexec/pixie_full, libexec/pixie_assistant.js, libexec/pixie_web, and runtime/ together in one extracted directory. Run the native Pi TUI with ./pixie. The internal full supervisor starts the service topology:\n\n    ./libexec/pixie_full serve --assistant-config /absolute/path/to/assistant.json --web-config /absolute/path/to/pixie.json\n\nDo not co-install this product with pixie_cli: both provide the global pixie command and use the same Pi agent directory. Replace one installation only after stopping its service and TUI. The archive supplies its own Bun and Pi runtime; external Pi selection is rejected.\n`;
	}
}
...
async function buildArchitecture(
	...
	const cli = join(directory, "pixie_cli");
	const web = join(directory, "pixie_web");
	const full = join(directory, "pixie_full");
	await buildAssistantBundle(
		join(repositoryRoot, "assistant/src/serve.ts"),
		assistant,
		repositoryRoot,
		workRoot,
	...
	for (const [output, packagePath, stamped] of [
		[launcher, "./cmd/pixie", false],
		[cli, "./cmd/pixie-cli", true],
		[web, "./cmd", true],
		[full, "./cmd/pixie-full", true],
	] as const) {
		await run(
			[
				"go",
	...
	for (const binary of [launcher, cli, web, full])
		await assertLinuxExecutable(binary, architecture);
	...
	return { assistant, launcher, cli, web, full };
}
...
async function archiveProduct(
	...
	switch (product) {
		case "pixie_web":
			...
			break;
		case "pixie_cli":
			entrypointPath = binaries.cli;
			entries.push(
				await copyStageFile(stage, "pixie", binaries.launcher, 0o755),
				await copyStageFile(stage, "pixie_cli", binaries.cli, 0o755),
		...
		case "pixie":
			...
			entries.push(
				await copyStageFile(stage, "pixie", binaries.launcher, 0o755),
				await copyStageFile(stage, "libexec/pixie_full", binaries.full, 0o755),
				await copyStageFile(stage, "libexec/pixie_assistant.js", binaries.assistant, 0o644),
				await copyStageFile(stage, "libexec/pixie_web", binaries.web, 0o755),
			);
			break;
	}
	if (productUsesRuntime(product)) {
