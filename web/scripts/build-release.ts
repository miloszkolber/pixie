...
async function buildBinary(
	...
	const moduleRoot =
		variant === "assistant" ? join(repositoryRoot, "assistant") : join(repositoryRoot, "web");
	const packagePath = variant === "assistant" ? "./cmd" : "./cmd";
	await run(
		[
			"go",
			"-C",
...
async function archiveBinary(
	...
	await copyFile(join(repositoryRoot, "web/systemd", unit), join(stage, unit));
	await copyFile(join(repositoryRoot, "web/systemd", config), join(stage, config));
