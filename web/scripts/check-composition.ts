...
function checkDockerfile(
	...
	if (dockerfile === undefined) {
		violations.push(
			"web/Dockerfile: Dockerfile is required for the controller-only composition check",
		);
		return missing;
	}
	...
	if (hasBuildRecipe) {
		...
		if (!missing.controllerOnlyBuild) {
			violations.push(
				"web/Dockerfile: controller image build must use the controller build tag and drop the assistant module",
			);
		}
	} else {
	...
	if (launch === "") {
		violations.push(
			"web/Dockerfile: final application stage must declare an entrypoint or command",
		);
		return missing;
	}
...
export async function collectCompositionInput(
	...
	const packageCommandSources = await collectSourceTree(repositoryRoot, "web/cmd");
	const packageInternalSources = await collectSourceTree(repositoryRoot, "web/internal");
	const packageWebuiTree = await collectSourceTree(repositoryRoot, "web/webui");
	...
	const packageGoModText = await readFile(resolve(repositoryRoot, "web/go.mod"), "utf8").catch(
		() => undefined,
	);
