...
// sanitizedStderrSummary bounds an externally supplied summary and drops any
// entry that is not a parseable time plus sanitized text.
func sanitizedStderrSummary(value *StderrSummary) *StderrSummary {
	if value == nil {
		return nil
	}
	result := &StderrSummary{Dropped: value.Dropped}
	entries := value.Entries
	if len(entries) > DefaultStderrMaxLines {
		entries = entries[len(entries)-DefaultStderrMaxLines:]
	}
	for _, entry := range entries {
		parsed, err := time.Parse(time.RFC3339, entry.At)
		if err != nil {
			continue
		}
		text := SanitizeDiagnosticText(entry.Text, MaxStderrLineBytes)
		if text == "" {
			continue
		}
		result.Entries = append(result.Entries, StderrEntry{At: parsed.UTC().Format(time.RFC3339), Text: text})
		result.Bytes += len(text)
	}
	// A caller-supplied summary can exceed the advertised byte budget even when
	// its line count is within bounds. Drop oldest entries until both bounds
	// hold so the export never exceeds the documented retention.
	for result.Bytes > DefaultStderrMaxBytes && len(result.Entries) > 0 {
		oldest := result.Entries[0]
		result.Entries = result.Entries[1:]
		result.Bytes -= len(oldest.Text)
		result.Dropped++
	}
	result.Retained = len(result.Entries)
