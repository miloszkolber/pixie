...
<span class="project-chat-history" {@attach mewa(dropdownBehavior)}>
	<Button
		variant="ghost"
		size="icon-sm"
	...
	<div bind:this={menu} id={menuId} popover="auto" role="menu" class="dropdown-menu-content history-menu" ontoggle={(event) => { menuOpen = event.newState === "open"; }}>
		...
		{#if closed.length === 0}
			<p class="u-px-sm u-py-xs u-text-text-muted tr-text-metadata">No recently closed chats</p>
		{:else}
			{#each closed as chat (chat.sessionId)}
				<div role="group" data-testid="closed-chat-row" class="u-flex u-items-center">
					<button type="button" role="menuitem" class="dropdown-menu-item u-min-w-0 u-flex-1" data-testid="closed-chat-item" data-session-id={chat.sessionId} onclick={() => { closeMenu(); void openChatInTab(projectAreaId, chat.sessionId); }}>
						<span class="u-flex-1 u-truncate">{chat.title}</span><span class="dropdown-menu-shortcut">{relativeTime(chat.closedAt)}</span><Icon name="rotate-ccw" size={14} />
					</button>
					<button type="button" role="menuitem" class="dropdown-menu-item u-shrink-0 u-px-xs" disabled={!canRename} title={renameUnavailable ?? "Rename chat"} aria-label={renameUnavailable ? `Rename ${chat.title}: ${renameUnavailable}` : `Rename ${chat.title}`} onclick={() => { closeMenu(); renameTarget = { projectId: projectAreaId, sessionId: chat.sessionId, title: chat.title }; }}><Icon name="pencil" size={14} /></button>
					<button type="button" role="menuitem" class="dropdown-menu-item u-shrink-0 u-px-xs" disabled={!canArchive} title={archiveUnavailable ?? "Archive chat"} aria-label={archiveUnavailable ? `Archive ${chat.title}: ${archiveUnavailable}` : `Archive ${chat.title}`} onclick={() => { closeMenu(); archiveTarget = { projectId: projectAreaId, sessionId: chat.sessionId, title: chat.title }; }}><Icon name="archive" size={14} /></button>
					<button type="button" role="menuitem" class="dropdown-menu-item u-shrink-0 u-px-xs" data-variant="destructive" data-testid="closed-chat-delete" disabled={!canDelete} title={deleteUnavailable ?? "Move chat to trash"} aria-label={deleteUnavailable ? `Move ${chat.title} to trash: ${deleteUnavailable}` : `Move ${chat.title} to trash`} onclick={() => remove(chat)}><Icon name="trash-2" size={14} /></button>
				</div>
			{/each}
		{/if}
		{#each unavailableActions as reason}<p class="project-chat-history__reason u-px-sm u-py-xs u-text-text-muted tr-text-metadata">{reason}</p>{/each}
		...
		{#if showArchived && !canArchive}
			<p class="project-chat-history__reason u-px-sm u-py-xs u-text-text-muted tr-text-metadata">{archiveUnavailable}</p>
		{:else if showArchived && archivedLoading}
			<p role="status" aria-live="polite" class="u-px-sm u-py-xs u-text-text-muted tr-text-metadata">Loading archived chats…</p>
		...
		{:else if showArchived && archived.length === 0}
			<p class="u-px-sm u-py-xs u-text-text-muted tr-text-metadata">No archived chats</p>
		{:else if showArchived}
			{#each archived as session (session.sessionId)}
				<button type="button" role="menuitem" class="dropdown-menu-item" disabled={restoring === session.sessionId} aria-label={restoring === session.sessionId ? `Restoring ${session.title}` : `Restore ${session.title}`} onclick={() => restore(session.sessionId)}>
					<span class="u-min-w-0 u-flex-1 u-truncate">{session.title}</span><span class="dropdown-menu-shortcut">{relativeTime(session.updatedAt)}</span><Icon name="archive-restore" size={14} />
				</button>
			{/each}
		{/if}
	</div>
...
<style>
	.project-chat-history { display: contents; }
	.project-chat-history__reason { max-width: 18rem; }
