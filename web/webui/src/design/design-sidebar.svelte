...
>
	<p class="tr-text-eyebrow u-text-text-muted">Design</p>
	<p class="tr-text-metadata u-text-text-muted" data-testid="design-scope-notice">
		Instance-wide · every authorized Design reader can inspect this document.
	</p>

	<section aria-label="Source and upload status" class="u-flex u-min-w-0 u-flex-col u-gap-xs">
		<h3 class="tr-text-metadata u-text-text-muted">Source</h3>
		{#if metadata}
			<p class="tr-text-metadata u-text-text-default" data-testid="design-document-metadata">
				{metadata.name} · {metadata.sizeLabel} · {metadata.dateLabel} · {metadata.countsLabel}
			</p>
			<p class="tr-text-metadata u-text-text-muted">{metadata.scopeNotice}</p>
		{:else}
			<p class="tr-text-metadata u-text-text-muted" data-testid="design-document-empty">
		...
		{/if}
		<p class="tr-text-metadata u-text-text-muted" data-testid="design-upload-status">
		...
		{#if upload.phase === "uploading"}
			...
			<p class="tr-text-metadata u-text-text-muted" data-testid="design-upload-metadata">
		...
		{/if}
		{#if uploadError}
			<p role="alert" class="tr-text-metadata u-text-feedback-error" data-testid="design-upload-error">
				{uploadError}
			</p>
		{/if}
		{#if state.document}
			<p class="tr-text-metadata u-text-text-muted" data-testid="design-removal-metadata">
			...
			<p class="tr-text-metadata u-text-text-muted" data-testid="design-removal-confirmation">
				{removal.title} {removal.message} {removal.copiesNotice}
			</p>
		{/if}
	</section>

	<section aria-label="Pages, layers, and frame candidates" class="u-flex u-min-w-0 u-flex-col u-gap-xs">
		<h3 class="tr-text-metadata u-text-text-muted">Pages · layers · frames</h3>
		{#if inspector.availability.available}
			<ul class="u-flex u-min-w-0 u-flex-col u-gap-0.5" aria-label="Design pages">
				{#each inspector.pages as page}
					<li
						class="u-truncate tr-text-metadata u-text-text-default"
			...
			<ul class="u-flex u-min-w-0 u-flex-col u-gap-0.5" aria-label="Design layers">
				{#each inspector.layers as layer}
					<li
						class="u-truncate tr-text-metadata u-text-text-muted"
			...
			<ul class="u-flex u-min-w-0 u-flex-col u-gap-0.5" aria-label="Design frame candidates">
			...
			<p class="tr-text-metadata u-text-text-muted" data-testid="design-inspector-counts">
				{inspector.pageCount} pages · {inspector.nodeCount} nodes · shared focus revision {inspector.sharedRevision}
			</p>
		{:else}
			<p
				role="status"
				class="tr-text-metadata u-text-text-muted"
			...
			>
				...
				{#each inspector.availability.diagnostics as diagnostic}
					<span class="design-sidebar-diagnostic">{diagnostic}</span>
				{/each}
			</p>
		{/if}
		{#if inspectorError}
			<p role="alert" class="tr-text-metadata u-text-feedback-error" data-testid="design-inspector-error">
				{inspectorError}
			</p>
		{/if}
	</section>

	<section aria-label="Shared focus" class="u-flex u-min-w-0 u-flex-col u-gap-xs">
		<h3 class="tr-text-metadata u-text-text-muted">Shared focus</h3>
		<p class="tr-text-metadata u-text-text-muted">Private browsing never changes shared focus.</p>
		<button
			type="button"
			class="btn"
			data-variant="outline"
		...
		{#if !publish.enabled && publish.reason}
			<p class="tr-text-metadata u-text-text-muted" data-testid="design-shared-focus-reason">
				{publish.reason}
			</p>
		{/if}
		{#if publish.optimisticRevision !== null}
			<p class="tr-text-metadata u-text-text-muted" data-testid="design-shared-focus-revision">
				Optimistic revision {publish.optimisticRevision}
			</p>
		{/if}
	</section>

	<section aria-label="Chat reference" class="u-flex u-min-w-0 u-flex-col u-gap-xs">
		<h3 class="tr-text-metadata u-text-text-muted">Chat reference</h3>
		{#if draftReference}
			<p class="u-truncate tr-text-metadata u-text-text-muted" data-testid="design-draft-reference-preview">
				{draftReference}
			</p>
		{/if}
		<button
		...
		<p class="tr-text-metadata u-text-text-muted">{draftAction.hint}</p>
		{#if !draftAction.enabled && draftAction.reason}
			<p class="tr-text-metadata u-text-text-muted" data-testid="design-draft-reference-reason">
				{draftAction.reason}
			</p>
		{/if}
	</section>
