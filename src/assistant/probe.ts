/**
 * Verified external Pi SDK selection for the Bun assistant host.
 *
 * It verifies the operator-selected package and loads only its verified
 * public entrypoint so the host uses the SDK in-process without bundling
 * or resolving Pi from the workspace, global installation, or current
 * working directory.
 */
