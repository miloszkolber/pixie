...
<section aria-label="Schedule details" data-testid="schedule-detail" class="u-flex u-min-w-0 u-flex-col u-gap-md">
	...
	{:else if missing}
		<div data-testid="schedule-missing" class="u-flex u-min-w-0 u-flex-col u-gap-sm">
			<p role="alert" class="tr-text-ui">The selected schedule is unavailable. It may have been deleted in another view.</p>
			<p class="tr-text-metadata u-text-text-muted">The ledger for the remaining schedules is unchanged. Choose another schedule in the list; this view does not redirect.</p>
			{#if requestedId}<p class="schedule-value tr-text-metadata u-text-text-muted">Missing schedule: {requestedId}</p>{/if}
		</div>
	{:else if !selected}
		<div data-testid="schedule-empty" class="u-flex u-min-w-0 u-flex-col u-gap-sm">
			<p class="tr-text-ui u-text-text-muted">Select a schedule in the list to inspect its timing, next occurrence and run ledger.</p>
		</div>
	{:else}
		<div class="u-flex u-min-w-0 u-flex-col u-gap-md">
			...
			<ol class="schedule-run-list" data-testid="schedule-runs">
...
<style>
	.schedule-run-list > * + * { border-top: 1px solid var(--border-muted); }
