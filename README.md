...
| `pixie` | Host Pi owner. Bundles Bun `1.4.0`, the Pi SDK and the assistant host connector. Bare `pixie` is the regular bundled native Pi TUI; run its host with `pixie serve --config ABS`. The TUI runs concurrently with the server. |
