...
const token = (value: unknown) => createHmac("sha256", tokenKey).update(JSON.stringify(value)).digest("hex");
...
function parseSettings(raw: string | undefined): Settings {
	...
	for (const field of ["packages", "extensions"]) {
		if (value[field] !== undefined && !Array.isArray(value[field])) throw new Error("Invalid settings");
	}
	return value;
}

function override(patterns: string[], path: string, base: string, enabled: boolean): string[] {
	if (patterns.some((pattern) => typeof pattern !== "string")) throw new Error("Invalid native resource filters");
	return [...patterns.filter((pattern) => !(["+", "-"].includes(pattern[0] ?? "") &&
		resolve(base, pattern.slice(1)) === resolve(base, path))), `${enabled ? "+" : "-"}${path}`];
}

export async function configureExtension(agentDir: string, cwd: string, request: {
	scope: ExtensionScope; resourceKey: string; expectedRevision: string; enabled: boolean; confirmed: boolean;
}) {
	if (!["user", "project"].includes(request.scope) || typeof request.enabled !== "boolean" || request.confirmed !== true)
		throw new Error("Confirm the scoped native configuration change");
	const source = SettingsManager.create(cwd, agentDir);
	if (source.drainErrors().length) throw new Error("Native settings are unreadable. No configuration saved.");
	...
	if (!resource || resource.metadata.scope !== request.scope) throw new Error("Native resource is no longer available. Refresh inventory.");
	const next = structuredClone(snapshots[scope]);
	let field: "packages" | "extensions" = "extensions";
	if (resource.metadata.origin === "package") {
		field = "packages";
		const index = next.packages?.findIndex((entry) => (typeof entry === "string" ? entry : entry.source) === resource.metadata.source) ?? -1;
		if (index < 0 || !next.packages || !resource.metadata.baseDir) throw new Error("Manage this resource through native Pi configuration");
		...
		next.packages[index] = { ...entry, extensions: override(patterns,
			relative(resource.metadata.baseDir, resource.path).split("\\").join("/"), resource.metadata.baseDir, request.enabled) };
	} else {
		next.extensions = override(next.extensions ?? [], resource.path,
			request.scope === "user" ? agentDir : join(cwd, ".pi"), request.enabled);
	}
	...
	const previewSettings = SettingsManager.fromStorage({ withLock: (target, fn) => {
		fn(JSON.stringify(target === scope ? next : snapshots[target]));
	} });
	const preview = (await new DefaultPackageManager({ cwd, agentDir, settingsManager: previewSettings }).resolve(async () => "skip")).extensions;
	const afterByKey = new Map(preview.map((item) => [extensionResourceKey(item), item]));
	const changed = afterByKey.get(request.resourceKey);
	if (!changed || changed.enabled !== request.enabled) throw new Error("Native filters cannot apply this change. No configuration saved.");
	if ([...beforeByKey].some(([key, before]) => key !== request.resourceKey && afterByKey.get(key)?.enabled !== before.enabled) ||
	...
	let writing = false;
	let saved = false;
	const manager = SettingsManager.fromStorage({ withLock: (target, fn) => {
		if (!writing) { fn(JSON.stringify(snapshots[target])); return; }
		if (target !== scope) throw new Error("Unexpected settings scope");
		const path = target === "global" ? join(agentDir, "settings.json") : join(cwd, ".pi", "settings.json");
	...
	if (!saved) throw new Error("Native settings save was not confirmed or conflicted. Refresh inventory before retrying.");
	return { saved: true, loaded: false, reload: "deferred" as const, reason: "sdk-loader-install-policy" as const,
