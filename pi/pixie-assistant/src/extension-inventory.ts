...
export async function extensionInventory(
	agentDir: string,
	cwd: string,
	...
	if (
		...
		(loaded?.extensions.length ?? 0) > limit ||
		(loaded?.errors.length ?? 0) > limit ||
		(globalSettings.extensions?.length ?? 0) > limit ||
				...
				: !loaded
					...
					: resources.some(
								(resource) =>
									...
									loaded.errors.some((error) => error.path === resource.path),
							)
						? "failed"
						: loaded.extensions.some(
									(extension) =>
										extension.sourceInfo.source === pkg.source &&
										extension.sourceInfo.scope === pkg.scope,
								)
							? "loaded"
							: "not-loaded",
		})),
		...
		resources: resources.slice(0, limit).map((resource) => ({
			...
			state: !loaded
				...
				: loaded.errors.some((error) => error.path === resource.path)
					...
					: loaded.extensions.some(
								(extension) =>
									(extension.resolvedPath === resource.path || extension.path === resource.path) &&
									extension.sourceInfo.scope === resource.metadata.scope,
							)
						? "loaded"
						: "not-loaded",
		})),
		extensions: (loaded?.extensions ?? []).slice(0, limit).map((extension) => ({
			path: inventoryReference(extension.path),
			resolvedPath: inventoryReference(extension.resolvedPath),
