...
test("known mutation retries are status reads and never start a second Pi operation", async () => {
	...
	await runtime.executePrompt({ mutationId: "mutation-1", content: [{ type: "text", text: "hello" }] }, invoke);
	const replay = await runtime.executePrompt({ mutationId: "mutation-1", content: [{ type: "text", text: "hello" }] }, invoke);
	 expect(calls).toBe(1);
...
test("passive state and drafts rebind across a native replacement without losing draft text", () => {
	...
	const applied = runtime.applyDraft("browser-a", { mutationId: "edit-1", expectedRevision: draft.draft.revision, content: "keep this and more" });
	...
	const reopened = runtime.reopen({ ...options(), bootId: "boot-2" });
	 ...
	 expect(runtime.getDraft("browser-a").draft).toMatchObject({ content: "keep this and more", revision: 1, continuityRevision: 1 });
	 expect(runtime.passive.title).toBe("");
});

test("runtime wiring keeps history reads bounded and capacity decisions explicit", async () => {
	const wiring = new RuntimeWiring({ agentDir: "/tmp/runtime-wiring-test", bootId: "boot-1", persist: false, maxOutboxEntries: 1 });
	 expect(wiring.canCreate()).toEqual({ ok: true, value: true });
	 expect((await wiring.scanHistory('{"type":"message","id":"one"}\nnot-json\n')).unknownRecords).toHaveLength(1);
	 expect(wiring.catalog([{ sessionId: "one", known: true }, { sessionId: "one", known: false }])).toMatchObject({
