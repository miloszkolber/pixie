...
browser wait --timeout 60000 --text "Inspect the workspace" >/dev/null
browser wait --timeout 60000 --text "1 of 2 complete" >/dev/null
...
browser wait --timeout 60000 --fn "document.activeElement?.getAttribute('data-testid') === 'session-plan-trigger'" >/dev/null
...
browser wait --timeout 60000 --fn 'document.querySelector("[data-testid=session-thinking-select]") !== null' >/dev/null
browser select '[data-testid=session-thinking-select]' low >/dev/null
browser wait --timeout 60000 --fn 'document.querySelector("[data-testid=session-thinking-select]")?.value === "low" && document.querySelector("[data-testid=session-thinking-select]")?.disabled === false' >/dev/null
...
browser wait --timeout 60000 --text "Uncommitted" >/dev/null
browser find role button click --name "Review scope: Uncommitted" >/dev/null
browser wait --timeout 60000 --text "Recent commit" >/dev/null
...
browser wait --timeout 60000 --text "history.txt" >/dev/null
browser click '[data-testid="change-item"][title="history.txt"]' >/dev/null
browser wait --timeout 60000 --text "after" >/dev/null
...
browser wait --timeout 60000 --fn "Array.from(document.querySelectorAll('[data-testid=file-node]')).some((node) => node.textContent?.trim() === 'README.md')" >/dev/null
browser find role button click --name "README.md" >/dev/null
browser wait --timeout 60000 --text "Welcome to acceptance." >/dev/null
...
browser wait --timeout 60000 --fn "Array.from(document.images).some((image) => image.alt === 'fixture.png' && image.complete && image.naturalWidth > 0)" >/dev/null
...
browser wait --timeout 60000 --text "Loaded answer" >/dev/null
browser wait --timeout 60000 --fn "document.querySelector('[data-testid=close-chat]') !== null" >/dev/null
browser wait --timeout 60000 --fn "document.querySelector('[data-testid=chat-input]')?.closest('[aria-hidden=true]') === null" >/dev/null
...
browser wait --timeout 60000 --fn "document.querySelector('[data-testid=chat-input]')?.value === ''" >/dev/null
browser wait --timeout 60000 --text "Continue" >/dev/null
browser wait --timeout 60000 --text "Partial reply" >/dev/null
...
browser wait --timeout 60000 --fn "document.querySelector('[data-testid=closed-chat-item]')?.offsetParent !== null" >/dev/null
...
for cycle in 1 2 3; do
 ...
 browser press Escape >/dev/null
 browser wait --timeout 60000 --fn "document.querySelector('[data-testid=session-plan-trigger]')?.popoverTargetElement?.matches(':popover-open') === false" >/dev/null
done
echo "UI acceptance: finish stream"
kill -USR1 "$fixture_pid"
browser wait --timeout 60000 --text "Partial reply complete." >/dev/null
...
browser wait --timeout 60000 --fn "document.querySelector('[data-testid=connection-status]')?.getAttribute('data-status') === 'disconnected'" >/dev/null
browser set offline off >/dev/null
browser wait --timeout 60000 --fn "document.querySelector('[data-testid=connection-status]')?.getAttribute('data-status') === 'connected'" >/dev/null
browser wait --timeout 60000 --fn "(document.body.innerText.match(/Partial reply complete\\./g) ?? []).length === 1" >/dev/null
...
browser wait --timeout 60000 --fn "document.querySelector('[data-testid=chat-attachment-dialog]')?.open === true" >/dev/null
browser press Escape >/dev/null
browser wait --timeout 60000 --fn "document.querySelector('[data-testid=chat-attachment-dialog]')?.open === false" >/dev/null
browser wait --timeout 60000 --fn "document.activeElement?.getAttribute('data-testid') === 'chat-attachment-chip'" >/dev/null
...
browser wait --timeout 60000 --fn "document.querySelector('[role=tab][aria-controls=settings-panel-providers]')?.getAttribute('aria-selected') === 'true'" >/dev/null
