...
const empty = (sessionId: string | null = null): NativeExtensionInventory => ({
	version: 1,
	context: { cwd: "/project", sessionId, reader: sessionId ? "session" : "configured-only" },
	packages: [],
	paths: [],
	resources: [],
	extensions: [],
...
test("native inventory requests retain exact target and reject stale replies, errors and wrong session results", async () => {
	const requests: {
		params: unknown;
		signal: AbortSignal;
		resolve: (value: NativeExtensionInventory) => void;
		reject: (reason: unknown) => void;
	}[] = [];
	const transport = {
		request: (method: string, params: unknown, options: { signal: AbortSignal }) => {
			expect(method).toBe("pi.nativeExtensions");
			return new Promise<NativeExtensionInventory>((resolve, reject) =>
				requests.push({ params, signal: options.signal, resolve, reject }),
			);
		},
	...
	requests[1]!.resolve(empty("chat"));
	await second;
	requests[0]!.reject(new Error("credential must not appear"));
	await first;
	expect(model.state.getState().inventory?.context.sessionId).toBe("chat");
	const wrong = model.load();
	requests[2]!.resolve(empty("foreign"));
	await wrong;
	expect(model.state.getState().inventory).toBeNull();
	expect(model.state.getState().error).toContain("unavailable");
	const disposed = model.load();
	model.dispose();
	requests[3]!.resolve(empty("chat"));
	await disposed;
	expect(model.state.getState().inventory).toBeNull();
});
...
test("Extensions screen renders native metadata, scoped unavailable and failed states without controls or unsafe HTML", async () => {
	...
	inventory.extensions.push({
		path: "<script>alert(1)</script>",
		resolvedPath: "/project/plugin.js",
		source: { scope: "project", origin: "top-level", source: "local" },
		name: null,
		version: null,
		tools: ["unknown_tool"],
	...
	expect(html).toContain("unknown-command");
	...
	expect(failed).not.toContain("No configured packages");
...
test("native configuration requires confirmation, saves scoped revisions and distinguishes saved configuration from deferred loading", async () => {
	...
	inventory.resources.push({
		path: "/project/.pi/extensions/unknown.js",
		source: "auto",
		scope: "project",
		origin: "top-level",
		enabled: true,
		state: "loaded",
		resourceKey: "c".repeat(64),
		...
		request: async (method: string, params: unknown) => {
			...
			if (method === "pi.nativeExtensionConfigure") {
				...
				return {
					...
					loaded: false,
					reload: "deferred",
					reason: "sdk-loader-install-policy",
				};
			}
			if (method === "pi.nativeExtensionReload")
				return { loaded: false, reload: "deferred", reason: "session-busy" };
			throw new Error(`Unexpected method: ${method}`);
		},
	} as unknown as Pick<WsTransport, "request">;
	const model = new ExtensionsModel(
		{ projectId: "p", root: "/project", sessionId: "chat" },
		transport,
	);
	await model.load();
	...
	await model.configure(resource, (message) => {
		expect(message).toContain("project settings");
		expect(message).toContain("host user's authority");
		return true;
	});
	...
	expect(model.state.getState().notice).toContain(
		"Configuration saved. Loaded extensions are unchanged",
	);
	expect(model.state.getState().inventory?.resources[0]).toMatchObject({
		enabled: false,
		state: "loaded",
	});
	await model.requestReload();
	expect(model.state.getState().notice).toContain("Nothing was stopped");
	const reloadedTransport = {
	...
	const reloadedModel = new ExtensionsModel(
		{ projectId: "p", root: "/project", sessionId: "chat" },
		reloadedTransport,
	);
	await reloadedModel.load();
	await reloadedModel.requestReload();
	expect(reloadedModel.state.getState().notice).toContain(
		"reopened with the saved native configuration",
	);
	failSave = true;
...
test("configuration buttons name scope and show real operation progress rather than pretending a deferred reload ran", async () => {
	...
	const html = await renderSvelte("src/settings/sections/extensions-inventory.svelte", {
		inventory,
		label: "Project: A",
		busy: "saving",
		canRequestReload: true,
	});
	expect(html).toContain("Enable on next load (project)");
	expect(html).toContain("Saving native configuration");
	expect(html).toContain("disabled");
	const checking = await renderSvelte("src/settings/sections/extensions-inventory.svelte", {
		inventory,
		label: "Project: A",
