...
import { extensionInventory, inventoryReference } from "../../../pi/pixie-assistant/src/extension-inventory.ts";
...
async function fixture() {
	...
	const agentDir = join(root, "agent"), cwd = join(root, "project"), pkg = join(root, "unfamiliar");
	await mkdir(join(agentDir, "extensions"), { recursive: true });
	await mkdir(join(cwd, ".pi"), { recursive: true });
	await mkdir(pkg);
	await writeFile(join(pkg, "package.json"), JSON.stringify({ name: "unfamiliar-native", version: "1.2.3", pi: { extensions: ["index.js"] }, secret: "manifest-secret" }));
	await writeFile(join(pkg, "index.js"), `export default pi => {
...
test("read-only native resolution inventories unknown packages, files, missing sources and project precedence without evaluating code", async () => {
	...
	await writeFile(join(agentDir, "extensions", "unknown.js"), `import { writeFileSync } from "node:fs"; writeFileSync(${JSON.stringify(marker)}, "evaluated"); export default () => {};`);
	...
	await writeFile(packageProbe, `import { appendFileSync } from "node:fs";
	...
	const config = JSON.stringify({ packages: [pkg, missing, "npm:pixie-fixture-never-installed-482@1.0.0"], extensions: [missingFile], npmCommand: ["/usr/bin/env", process.execPath, packageProbe] });
	...
	expect(inventory.resources.filter((item) => item.source === pkg).map((item) => item.scope)).toEqual(["project"]);
...
test("actual resident inventory reports native contributions, failed loads, unknown support and configured divergence without raw diagnostics", async () => {
	...
	await writeFile(join(agentDir, "extensions", "broken.js"), 'export default () => { throw new Error("Authorization: Bearer fixture-credential"); };');
	...
	expect(inventory.extensions.find((item) => item.source.source === pkg)).toMatchObject({
		version: null, tools: ["unfamiliar_native"], commands: ["unfamiliar-command"], interfaceSupport: "unknown",
	});
	expect(inventory.errors).toEqual([{ path: join(agentDir, "extensions", "broken.js"), code: "load-failed" }]);
...
test("malformed native settings produce an explicit incomplete inventory and references redact URL credentials", async () => {
	...
	expect(inventoryReference("git:https://user:private-key@example.test/pkg?token=secret#private")).toBe("git:https://example.test/pkg");
});

test("assistant inventory requests never reopen an evicted session and include the loaded SDK llama bootstrap", async () => {
	const { agentDir, cwd } = await fixture();
	const host = await startHost({ agentDir, secret: "inventory-fixture-secret", port: 0, llama: true });
	...
	const socket = new WebSocket(`ws://127.0.0.1:${host.server.port}/pi`, { headers: { Authorization: "Bearer inventory-fixture-secret" } });
	...
	const response = new Promise<unknown>((resolve) => { socket.onmessage = (event) => resolve(JSON.parse(String(event.data))); });
	socket.send(JSON.stringify({ id: 1, method: "pi.extensions.list", params: { sessionId: id, cwd } }));
	expect(await response).toMatchObject({ result: { context: { reader: "not-resident", sessionId: id }, extensions: [] } });
