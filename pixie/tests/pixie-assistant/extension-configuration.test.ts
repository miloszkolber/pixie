...
async function fixture() {
	...
	await writeFile(
		...
		JSON.stringify({ extensions: [], future: { keep: "unchanged" } }),
	);
...
async function requestFor(
	...
	path: string,
	enabled: boolean,
): Promise<Parameters<typeof configureExtension>[2]> {
	const inventory = await extensionInventory(agentDir, cwd);
	const resource = inventory.resources.find((item) => item.path === path);
	if (!resource || (resource.scope !== "user" && resource.scope !== "project"))
		throw new Error("Missing native fixture resource");
	return {
		scope: resource.scope,
		resourceKey: resource.resourceKey,
		expectedRevision: inventory.configurationRevisions[resource.scope],
		enabled,
		confirmed: true,
	};
}
...
test("native user and project file filters save exact scope and leave loaded sessions unchanged", async () => {
	...
	expect(inventory.resources.find((item) => item.path === extension)).toMatchObject({
		enabled: false,
		state: "loaded",
	});
	...
	expect(
		enabled.session.getActiveToolNames().filter((name) => name === "native_configuration_probe"),
	).toHaveLength(1);
});
...
test("unsupported native package-file filters and unconfirmed changes leave settings untouched", async () => {
	...
	await expect(configureExtension(agentDir, cwd, { ...request, confirmed: false })).rejects.toThrow(
		"Confirm",
	);
...
test("native package filters enable only the selected resource and preserve other resource filters and unknown keys", async () => {
	...
	await writeFile(
		...
		JSON.stringify({
			name: "unfamiliar-configurable",
			version: "1.0.0",
			pi: { extensions: ["one.js", "two.js"] },
		}),
	);
	await writeFile(
		join(agentDir, "settings.json"),
		JSON.stringify({
			packages: [{ source: pkg, extensions: [], skills: ["kept/**"], future: "keep" }],
			untouched: true,
		}),
	);
	await configureExtension(
		agentDir,
		cwd,
		await requestFor(agentDir, cwd, join(pkg, "one.js"), true),
	);
	const current = JSON.parse(await readFile(join(agentDir, "settings.json"), "utf8"));
	expect(current).toMatchObject({
		untouched: true,
		packages: [
			{ source: pkg, extensions: ["!**", "+one.js"], skills: ["kept/**"], future: "keep" },
		],
	});
	const inventory = await extensionInventory(agentDir, cwd);
	expect(inventory.resources.find((item) => item.path === join(pkg, "one.js"))?.enabled).toBe(true);
	expect(inventory.resources.find((item) => item.path === join(pkg, "two.js"))?.enabled).toBe(
		false,
	);
});
...
test("write-time compare-and-set preserves unrelated concurrent edits and rejects concurrent filter edits", async () => {
	...
	try {
		for (const conflict of [false, true]) {
			...
			if (conflict)
				await expect(configureExtension(agentDir, cwd, request)).rejects.toThrow("conflicted");
			...
			if (conflict)
				expect(JSON.parse(await readFile(path, "utf8")).extensions).toContain("!concurrent.js");
		}
	} finally {
		SettingsManager.prototype.setExtensionPaths = originalSetter;
	}
});
...
test("idle reload reopens with saved configuration while another session's active run is untouched", async () => {
	...
	const idle = await sessions.create(cwd),
		active = await sessions.create(cwd);
...
test("saving an enabled extension does not claim it loaded or roll back configuration after a later native load failure", async () => {
	...
	await configureExtension(agentDir, cwd, await requestFor(agentDir, cwd, extension, false));
	const saved = await configureExtension(
		agentDir,
		cwd,
		await requestFor(agentDir, cwd, extension, true),
	);
	...
	expect(inventory.resources.find((resource) => resource.path === extension)).toMatchObject({
		enabled: true,
		state: "failed",
	});
});
