...
export function nativeStateLabel(state: NativeResourceState): string {
	return {
		loaded: "Loaded",
		failed: "Load failed",
		"not-loaded": "Not loaded in this context",
		"not-observed": "Loaded state unavailable",
		missing: "Source not installed or missing",
	}[state];
}
export function nativeReaderLabel(reader: NativeExtensionInventory["context"]["reader"]): string {
	return {
		session: "Loaded in the selected session",
		service: "Loaded in the assistant service context, not a project session",
		"not-resident": "The selected session is not resident. It was not opened for inspection.",
		"configured-only":
			"Configured project resources only. Select an open chat to inspect its loaded extensions.",
	}[reader];
}
...
export class ExtensionsModel {
	...
	constructor(
		...
		private transport: Pick<WsTransport, "request">,
	) {}
	async load(): Promise<void> {
		...
		try {
			const inventory = await this.transport.request("pi.nativeExtensions", this.target, {
				signal: this.abort.signal,
			});
			if (generation !== this.generation) return;
			if (
				inventory.version !== 1 ||
				inventory.context.sessionId !== (this.target.sessionId ?? null) ||
				(this.target.root && inventory.context.cwd !== this.target.root)
			)
			...
			if (generation === this.generation)
				this.state.setState({
					...
					loading: false,
					error:
						"Native extension inventory unavailable. Retry after checking the assistant connection.",
				});
		}
	}
	async configure(
		resource: NativeExtensionInventory["resources"][number],
		...
		if (
			!confirm(
				`${enabled ? "Enable" : "Disable"} this native resource for the next load in ${scope} settings?\n${resource.path}\n\nEnabling runs extension code with the host user's authority on a subsequent native load. No installation or in-process reload will occur. Current sessions remain unchanged.`,
			)
		)
			return;
		const generation = ++this.generation;
		...
		try {
			...
			if (generation !== this.generation) return;
			if (result.saved !== true || result.loaded !== false || result.reload !== "deferred")
			...
			if (generation === this.generation)
				this.state.setState({
					...
					inventory: null,
					error:
						"Save outcome not confirmed. Configuration may have changed or conflicted. Refresh inventory before retrying. Loaded sessions were not reloaded.",
				});
		}
	}
	async requestReload(): Promise<void> {
		if (!this.target.sessionId || this.state.getState().busy || this.state.getState().loading)
			return;
		const generation = ++this.generation;
		this.state.setState({ busy: "checking-reload", error: null, notice: null });
		try {
			const result = await this.transport.request("pi.nativeExtensionReload", {
				...this.target,
				sessionId: this.target.sessionId,
			});
			if (generation !== this.generation) return;
			if (result.reload !== "deferred" && result.reload !== "reloaded")
				throw new Error("Unexpected reload outcome");
			this.state.setState({ busy: null, notice: reloadNotice(result) });
			await this.load();
		} catch {
			if (generation === this.generation)
				this.state.setState({
					busy: null,
					error: "Reload request outcome unavailable. No reload success is confirmed.",
				});
		}
	}
	dispose(): void {
