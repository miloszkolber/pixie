...
let project = $derived($appStore.projects.find((project) => project.id === (area?.projectId ?? $appStore.selectedProjectId)));
let sessionId = $derived(area && tab?.kind === "chat" ? tab.sessionId : undefined);
let target = $derived(project ? { projectId: project.id, root: project.roots[0] ?? "", ...(sessionId ? { sessionId } : {}) } : {});
let label = $derived(project ? `Project: ${project.name}${sessionId ? ` · Session: ${sessionId}` : " · No session selected"}` : "Assistant service context · No project selected");
</script>
