...
const rootManifestPath = resolve(import.meta.dir, "../../package.json");
...
test("root package has no npm publication surface", async () => {
...
test("release retries compare immutable asset hashes instead of clobbering", async () => {
	...
	expect(workflow).toContain("complete four-archive payload; refusing to publish");
	expect(workflow).toContain("contains assets outside the complete four-archive payload");
...
test("release stages each architecture natively, then merges the exact four-archive set before evidence", async () => {
	...
	expect(merge).toContain("(.artifacts | length == 4)");
...
test("release produces native arm64 performance and probe evidence before the gate and publication", async () => {
	...
	expect(arm64).toContain("products/pixie/pixie");
	expect(arm64).not.toContain('"$BIN_DIR/pixie"');
	expect(evidence).toContain("products/pixie/pixie");
