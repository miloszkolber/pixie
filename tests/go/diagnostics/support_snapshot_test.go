...
func TestSupportSnapshotSanitizesEnvironmentIdentity(t *testing.T) {
	...
	payload, err := diagnostics.MarshalSupportSnapshot(diagnostics.SupportSnapshotRuntime{}, nil)
	if err != nil {
		t.Fatal(err)
	}
	text := string(payload)
	for _, forbidden := range []string{"bearer-token-value", "alice"} {
		if strings.Contains(text, forbidden) {
			t.Fatalf("support snapshot leaked %q: %s", forbidden, text)
		}
	}
