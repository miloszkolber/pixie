...
// final-container reaping contract at the owned level: the published
// pixie_web image must run its controller under tini so PID 1 reaps orphaned managed-group
...
func TestDiagnosticsFinalContainerReapsChildren(t *testing.T) {
	...
	// with the pixie_web controller as its child, not replace tini with a bare entrypoint.
	if !strings.Contains(lastEntrypoint, "tini") || !strings.Contains(lastEntrypoint, "/app/pixie_web") {
		t.Fatalf("final ENTRYPOINT does not reap under tini: %q", lastEntrypoint)
	}
	if strings.TrimSpace(lastEntrypoint) == `ENTRYPOINT ["/app/pixie_web"]` {
