...
Three public Linux artifacts build from one repository: `pixie_web` (Go controller and UI), `pixie_cli` (bundled Bun, regular Pi TUI, Pi SDK and assistant host), and `pixie` (bundled Bun, regular Pi TUI, Pi SDK, assistant host and controller). `pixie_assistant` is an internal archive JS host (`libexec/pixie_assistant.js`, run by `runtime/bin/bun`), never a public product, unit or archive. Both Pi-bearing products expose `pixie` as the native Pi command; `pixie_cli serve --config ABS` starts the standalone host, while full-suite service mode is internal `libexec/pixie_full serve --assistant-config ABS --web-config ABS` so it does not take Pi's command namespace. `pixie_cli` and full `pixie` are mutually exclusive installations for the global `pixie` command and a shared Pi agent directory. `pixie_web` stays controller-only. The Pi-bearing runtime is pinned Bun 1.4.0 and never Node; Pi is never resolved from a global installation. The Go host's `pi --mode rpc` child processes and separate Bun administration bridge are removed; there is no Pi RPC child model or bridge sidecar. `pixie_web` consumes the narrow authenticated loopback host event protocol; that protocol is not a Pi execution fallback.
...
Docker has two products: the full `pixie` image runs the bundled Bun Pi runtime, host and controller against an explicitly mounted Pi state volume, while the `pixie_web` image is controller-only and connects to a separately installed `pixie_cli` host. The controller-only image never starts Pi and contains no Bun, Node or Pi package.
