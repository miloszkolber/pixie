...
- **Native error boundary (partial).** Request failures are normalized at both boundaries: the host redacts and bounds the native cause before replying (`assistant/src/host.ts`), and the controller maps a host error code to a fixed browser message (`shared/piprotocol/host_errors.go`, `web/internal/controller/websocket.go`) while logging only the redacted cause. Native update text carried inside session events is normalized at the controller boundary before persistence or projection: the known error-bearing fields and native status messages are redacted and bounded, and the native `agent_end` message array is replaced by a bounded stop/retry summary and dropped at that boundary (`web/internal/controller/session_events.go`, `web/internal/controller/pi_events.go`, `web/tests/go/controller/session_native_error_boundary_test.go`). The normalization is field-allowlisted, so native event fields outside those handled cases still pass through unchanged.
...
Host diagnostics redact secret-shaped and path-shaped values at their emission boundary: bearer tokens, credential-shaped values, endpoint URLs, absolute paths and long opaque tokens are replaced before text is printed. Child stderr passes through one supervisor redaction boundary: `pixie_full` writes each managed child's stderr only into a bounded, redacted ring whose console mirror receives the same already-redacted lines, so raw child text never reaches the supervisor console or a retained ring. The ring keeps line, count, byte and age caps, emits that tail with the child-exit log, and points at it from the doctor remediation. The controller owns no child process, so no child-stderr summary crosses the controller boundary (`assistant/src/log.ts:47-70`, `assistant/src/serve.ts:73-76`, `web/internal/diagnostics/stderr.go:11-20`, `web/internal/diagnostics/stderr.go:160-163`, `web/cmd/pixie-full/main.go:645-646`, `web/cmd/pixie-full/main.go:739-747`).
