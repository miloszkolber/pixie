...
`pixie_web doctor [--config ABS]` prints a read-only recovery report: a `summary=` value plus one line per check with a stable ID, status, code and fixed remediation text. The report carries no path, endpoint, credential or raw error (`internal/diagnostics/recovery.go:65-211`, `cmd/runtime.go:162-231`).
