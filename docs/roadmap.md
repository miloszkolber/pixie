...
- Robustness finding: when a provider stream stalls, `session.cancel` (`await s.abort()`) hangs and `session.load` on the affected session hangs with it, so Stop cannot unwind a hung provider call today. Restarting the host clears in-memory runs while persisted transcripts reload fine. A server-side abort timeout/force path is future work; it needs care not to corrupt run state.
