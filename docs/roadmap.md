...
- Published artifacts replace the checkout on clean machines: a versioned `@pixie_ai/pixie-assistant` npm package for the Pi service (`bunx @pixie_ai/pixie-assistant@<version>`) and the `ghcr.io/<owner>/pixie` container image for the application. Docker stays the primary application deployment; the Go binary plus a built web UI is the documented fallback.
