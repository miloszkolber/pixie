...
The Browser engine preference lives in Pixie app state (`browser.json`, default `chromium`) and the publisher honors it. Setting `obscura` requires `PIXIE_BROWSER_CDP` (a port such as `9222` or `host:port`, matching `obscura serve --port 9222`); without it the module degrades instead of silently using Chromium. While `chromium` is selected, a `PIXIE_BROWSER_CDP` override is ignored. Switching engines does not alter the model-facing API. There is no Tools UI toggle yet; see the [roadmap](roadmap.md).
