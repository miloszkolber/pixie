...
The amd64 startup measurements and native `--version`/`doctor` probes are real candidate-binary measurements. The reduced rows above remain unproduced: arm64 performance and arm64 platform digests, per-feature live coverage, packaged-binary lifecycle/uninstall/embedded-UI and full-host facade checks, Git tag/GitHub Release state, the published OCI tag and digests, registry provenance/SBOM and latest-promotion ancestry. arm64 performance and the `linux/arm64` OCI image build remain unproven.
