...
The repository root holds one Go module, the Bun workspace and lockfile, and shared build/test configuration. Source ownership follows the two public products:

| Path | Responsibility |
| --- | --- |
| `cmd/pixie/` | Native Pi TUI and host-service launcher |
| `cmd/pixie-web/` | Controller executable, configuration and operator commands |
| `cmd/internal/` | Launcher-only configuration, process-group and runtime helpers |
| `internal/` | Go application packages: controller, persistence, workspace, diagnostics and optional modules |
| `src/assistant/` | Archive-internal Bun host and Pi SDK integration |
| `src/shared/`, `piprotocol/`, `schema/` | Shared TypeScript and Go contracts and their source schema |
| `scripts/` | Repository checks, generation, release assembly and evidence tooling |
| `webui/` | Svelte frontend, its build tooling, vendored Mewa assets and Go embedding |
| `tests/go/` | Go package-level integration tests, including protocol contracts |
| `tests/assistant/`, `tests/shared/`, `tests/build/`, `tests/webui/` | Bun suites grouped by the code they exercise |
| `tests/internal/` | Test-only Go fixtures; Go prevents production packages from importing them |
| `tests/ui/`, `tests/deployment/`, `tests/performance/` | Browser acceptance, deployment checks and measurements |

Private Go unit tests stay beside their implementation. Cross-package tests belong under `tests/go/`; shared fixtures belong under `tests/internal/`. Both public binaries build directly from their command directories; no controller build tag is required.

The release builder has two public Linux product targets: controller-only `pixie_web` and bundled-Pi `pixie`. `pixie` exposes the regular native Pi TUI, bundles the pinned Bun `1.4.2` runtime `runtime/bin/bun` and Pi `0.86.1`, bundles no Node runtime, excludes Pi RPC, and blocks Pi self-update. Bun `1.3.14` cannot run Pi's bundle (`undici` `webidl.util.markAsUncloneable`), so `1.4.2` is required. The assistant is the bundled JavaScript host `libexec/pixie_assistant.js`, not a compiled executable, and remains archive-internal. Source builds and archive-layout checks do not prove a standalone distribution, credentialed Pi behavior, an approved Docker deployment, arm64 live lifecycle, or a published release.
...
`bun run test` runs the assistant, shared-contract and build Bun suites plus the Go suite. `bun run --cwd webui test` runs the frontend suite. Use `go test -race -count=1 ./...` and `go vet ./...` from the repository root for the single Go module.
...
- `bun run generate:config-schema` writes `docs/config-schema.json`, and `bun run check:config-schema` fails when that schema is stale relative to `cmd/pixie-web/config.go`, `src/assistant/serve.ts` and `cmd/internal/assistantconfig/config.go`. Controller serve and utility commands share the same strict configuration checks; pairing accepts either service's field shape to resolve its directories.
...
`bun run typecheck` covers all TypeScript source under `src/`, every Bun test, build tools and the frontend with the shared strict settings. `tsconfig.tools.json` checks every script under `scripts/` and `webui/scripts/`, including CLI entrypoints that tests invoke without importing.
...
Docker's frontend stage installs only the `@pixie/web` workspace and its dependencies. Pi and provider SDKs are needed by the native host archive, not by the controller image's frontend build.
