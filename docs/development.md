...
- `bun run generate:config-schema` writes `docs/config-schema.json`, and `bun run check:config-schema` fails when that schema is stale relative to `cmd/config.go`, `src/assistant/serve.ts` and `cmd/internal/assistantconfig/config.go`. Controller serve and utility commands share the same strict configuration checks; pairing accepts either service's field shape to resolve its directories.
...
`bun run typecheck` covers the assistant, shared protocol, build tools, tests and frontend. `tsconfig.tools.json` checks every script under `scripts/` and `webui/scripts/`, including CLI entrypoints that tests invoke without importing.
