...
import {
	...
	type NativeFileFingerprint,
	scanNativeHistory,
} from "./history/catalog.ts";
...
import { HostError, JsonStore } from "./storage.ts";
import type {
...
export class RuntimeWiring {
	...
	constructor(readonly options: RuntimeWiringOptions) {
		...
		if (options.persist !== false)
			this.store = new JsonStore(`${options.agentDir}/pixie/runtime-wiring.json`, () => ({
				version: RUNTIME_WIRING_VERSION,
				sessions: {},
			}));
	}
	...
	async ready(): Promise<void> {
		...
		this.loading ??= (async () => {
			...
			if (this.store) {
				...
				if (
					state.version !== RUNTIME_WIRING_VERSION ||
					!state.sessions ||
	...
	private async handleTransportFailure(failure: NativeTransportFailure): Promise<void> {
		...
		for (const [id, runtime] of this.runtimes) {
			if (
				!runtime.outbox.entries.some(
					(entry) => entry.status === "dispatching" || entry.status === "accepted",
				)
			)
				continue;
	...
	private async inspect(entry: ManagedSession): Promise<NativeSessionDetails> {
		...
		try {
			...
			if (inventory.trust.projectTrusted && inventory.trust.decision === null)
	...
	private resident(identity: RuntimeIdentity): boolean {
		return this._residency.residents.some(
			(resident) =>
				resident.sessionKey === identity.sessionKey && resident.generation === identity.generation,
		);
	}
	...
	private async persistRuntime(id: string, runtime: SessionRuntime): Promise<void> {
		...
		const encoded = JSON.stringify({
			version: RUNTIME_WIRING_VERSION,
			sessions: { ...Object.fromEntries(this.stored), [id]: record },
		});
	...
	async bind(entry: ManagedSession): Promise<SessionRuntime> {
		...
		if (current) {
			if (current.state.phase !== "ready")
				throw new HostError("Session generation is not idle; reconcile before reopening", -32003);
		...
		const generation =
			saved && saved.bootId !== this.options.bootId
				? saved.generation + 1
				: (saved?.generation ?? 0);
		const identity = {
	...
	canCreate(): RuntimeResult<true> {
		...
		return usage.residents >= this._residency.limits.maxResidents
			? {
					ok: false,
					error: {
						code: "resident-capacity",
						message: "Managed runtime resident capacity is full",
					},
				}
			: { ok: true, value: true };
	}
	...
	async prompt(
		...
		const known = input.mutationId
			? runtime.outbox.entries.some((candidate) => candidate.mutationId === input.mutationId)
			: false;
		...
		if (!known) {
			...
			const retainedBytes = runtime.outbox.entries.reduce(
				(total, candidate) => total + Buffer.byteLength(JSON.stringify(candidate.payload)),
				0,
			);
	...
	async configureModel(
		entry: ManagedSession,
		model: NativeModel,
	): Promise<RuntimeResult<SessionRuntime>> {
		const runtime = await this.bind(entry);
	...
	async draft(
		entry: ManagedSession,
		clientId: string,
		mutation: DraftMutation<string>,
	): Promise<RuntimeResult<DraftState<string>>> {
		const runtime = await this.bind(entry);
		...
		if (result.kind === "conflict")
			return {
				ok: false,
				error: { code: result.reason, message: "Draft revision or mutation identity conflicts" },
			};
	...
	async release(
		...
		const identity = this.identity(runtime);
		const began = toTui
			? releaseToTui(this._residency, identity, instruction)
			: releaseIdleRuntime(this._residency, identity);
		...
		if (this.options.sessions?.entries.has(entry.session.sessionId)) {
			return {
				ok: false,
				error: { code: "invalid-state", message: "Native session release did not complete" },
			};
		}
		const completed = toTui
			? completeTuiHandoff(this._residency, identity)
			: completeIdleRuntimeRelease(this._residency, identity);
	...
	private syncWork(runtime: SessionRuntime, entry: ManagedSession): void {
		const identity = this.identity(runtime);
		if (!this.resident(identity)) return;
		const resident = this._residency.residents.find(
		...
		const desired = {
			...
			pendingUiIds: new Set(
				(
					(this.options.sessions?.snapshot(entry).pendingDialogs as
		...
		for (const id of desired.activeWorkIds)
			if (!resident.work.activeWorkIds.includes(id))
				this.applyResidency({ type: "work.admit", identity, workId: id });
		for (const id of resident.work.activeWorkIds)
			if (!desired.activeWorkIds.has(id))
				this.applyResidency({ type: "work.settle", identity, workId: id });
		for (const id of desired.pendingUiIds)
			if (!resident.work.pendingUiIds.includes(id))
				this.applyResidency({ type: "ui.pin", identity, requestId: id });
		for (const id of resident.work.pendingUiIds)
			if (!desired.pendingUiIds.has(id))
				this.applyResidency({ type: "ui.unpin", identity, requestId: id });
		if (desired.livenessPinKeys.size && !resident.work.livenessPinKeys.includes("extension-work"))
			this.applyResidency({ type: "liveness.pin", identity, key: "extension-work" });
		if (!desired.livenessPinKeys.size && resident.work.livenessPinKeys.includes("extension-work"))
			this.applyResidency({ type: "liveness.unpin", identity, key: "extension-work" });
	}
	...
	onEvent(sessionId: string, event: unknown): void {
		...
		if (runtime.applyPassiveEvent(event as Record<string, unknown>))
			void this.persistRuntime(sessionId, runtime).catch(() => {});
	}
	...
	snapshot(sessionId: string, clientId?: string): SessionRuntimeReplay | undefined {
		...
		const resident = this._residency.residents.find(
			(candidate) => candidate.sessionKey === runtime.identity.sessionKey,
		);
		const work = resident
			? classifyRuntimeWork(resident.work)
			: { kind: "idle" as const, releaseable: true, reason: "settled" as const };
		return runtime.snapshot(
			clientId,
			resident
				? { phase: resident.phase, releaseable: work.releaseable, reason: work.reason }
				: undefined,
		);
	}
	...
	catalog(
		candidates: readonly Parameters<typeof buildBoundedCatalog>[0][number][],
		options?: Parameters<typeof buildBoundedCatalog>[1],
	): CatalogResult {
		return buildBoundedCatalog(candidates, options);
	}

	compareHistorySource(
	...
	async close(): Promise<void> {
		...
		if (this.store) {
			for (const [id, runtime] of this.runtimes)
				await this.persistRuntime(id, runtime).catch(() => {});
		}
